...
// 정렬을 우리가 안배웠기 때문에 문제를 고치겠습니다.
let value = "p2o4"
let sum = 0
if (!isNaN(value[0])) {sum += parseInt(value[0])}
if (!isNaN(value[1])) {sum += parseInt(value[0])}
if (!isNaN(value[2])) {sum += parseInt(value[0])}
if (!isNaN(value[3])) {sum += parseInt(value[0])}
console.log(sum) // 6

// 안배운 것을 써보자면, (!isNaN(v) 숫자인지 확인 <- 하지만 권고안함)
